...
def main(folder_name, file_extension):
    ...
    with open(gt_file, 'r') as file:
        ...
        with open("resultsCamshift.csv", 'w', newline='') as out_file:
            ...
            for image_file, gt_bbox in zip(image_files, gt_data):
                ...
                ret, track_window = cv2.CamShift(mask, (int(x), int(y), int(width), int(height)), term_criteria)
    ...
    with open("resultsfinalCamshift.csv", 'w', newline='') as out_file:
        writer = csv.writer(out_file)
